...
plot1 <- resid_fixed %>% ggplot(aes(`resid(fixed1)`)) + geom_histogram() + ggtitle("Fixed Effects Model") +xlab('Residuals') +theme_bw()
plot2 <- resid_random %>% ggplot(aes(`resid(random1)`)) + geom_histogram()+ ggtitle("Random Effects Model")+xlab('Residuals')+theme_bw()
library(cowplot)
...
ggpairs(final_data5, title="Correlations between Variables in the Model") +theme_bw()
