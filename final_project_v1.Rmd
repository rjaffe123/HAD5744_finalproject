...
final_data <- final_data %>% filter(!State %in% c("California", "Connecticut", "District of Columbia", "Minnestoa", "Washington", "Colorado"))
...
```{r}
final_data<-final_data %>% mutate(difference = round(interval(as.Date(as.character(Year), format = "%Y"),as.Date(date) ) /years(1)))
final_data <- final_data %>% filter(!(difference >3 | difference < -3) | is.na(difference))
...
final_data1 <- subset(final_data, is.na(date)) %>% mutate(difference = round(interval(as.Date(as.character(Year), format = "%Y"),as.Date(as.character(2014), format = "%Y") ) /years(1))) 
...
final_data <- final_data %>% filter(!is.na(date)) %>% rbind(final_data1)
```
...
final_data_grouped <- final_data_grouped %>% mutate(Unemployment_rate  = Unemployed / Civilian_labor_force,
                                                    ...
                                                    spending_ave_pergroup = unadjmedamt / count) %>% rename(death_rate= rate)
...
## Let's look at plots
```{r initial plots}
final_data_grouped %>% ggplot(aes(difference, death_rate, color=expan_group))+stat_summary(geom = 'line') +
...
## modeling
```{r}
didreg <- lm(death_rate ~ time*treated, data = final_data_grouped)
didreg1 = lm(death_rate ~ phyisican_perpop + spending_ave_pergroup + treated*time, data = final_data_grouped)
summary(didreg1)
```
